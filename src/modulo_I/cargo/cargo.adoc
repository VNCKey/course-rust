...
└── .gitignore          # Git configurado automáticamente
----
...
[source,bash]
----
# Compilación en modo desarrollo (más rápida, sin optimizaciones)
cargo build
...
[source,bash]
----
# Revisa errores sin generar el ejecutable (MUY rápido)
cargo check
----

NOTE: `cargo check` es hasta 10x más rápido que `cargo build` durante el desarrollo.
...
El corazón de tu proyecto. Define metadatos y dependencias:
