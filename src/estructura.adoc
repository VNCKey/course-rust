= 🦀 Ruta de Aprendizaje Rust 2026
:subtitle: De Principiante a Experto
:source-highlighter: rouge
:rouge-style: monokai
:icons: font
:toc: left
:toclevels: 4
:toc-title: Índice de Contenido
:sectnums:
:experimental:

[abstract]
.Sobre Esta Ruta
====
Esta es una ruta de aprendizaje completa y moderna de Rust para 2026, creada por la comunidad de *Rust Perú* 🇵🇪.

Cubre desde la instalación hasta temas avanzados como async, macros procedurales, unsafe, FFI y arquitectura de sistemas.

*Tiempo estimado*: 4-6 meses (dedicación diaria) +
*Nivel final*: Experto en Rust +
*Idioma*: Español
====

'''

[.text-center]
== 🎯 Niveles de Dominio

[cols="1,2,1,3", options="header"]
|===
|Nivel
|Descripción
|Módulos
|Duración Estimada

|🌱 *Principiante*
|Sintaxis básica, ownership, control de flujo
|0-6
|3-4 semanas

|🌿 *Intermedio*
|Genéricos, errores, colecciones, iteradores
|7-12
|4-6 semanas

|🌳 *Avanzado*
|Traits avanzados, lifetimes, concurrencia, async
|13-17
|6-8 semanas

|🔥 *Experto*
|Macros, unsafe, FFI, testing, optimización
|18-21
|4-6 semanas
|===

'''

= NIVEL PRINCIPIANTE 🌱

== Módulo 0: Preparación del Entorno

.*Objetivos*: Configurar el entorno y familiarizarse con las herramientas

En este módulo aprenderás a:

* Instalar Rust y rustup
* Configurar toolchains (`stable`, `beta`, `nightly`)
* Actualizar tu entorno de desarrollo
* Gestionar versiones con rustup

=== Herramientas Esenciales

* *Cargo*: Gestor de proyectos y dependencias
* *rustc*: Compilador de Rust
* *rustfmt*: Formateador de código
* *Clippy*: Linter y sugerencias
* *rust-analyzer*: Language Server Protocol (LSP)

=== Configuración del Editor

Tutoriales para configurar tus editores de código favoritos:

* VS Code + rust-analyzer
* Zed con extensiones Rust
* Neovim + rust-tools.nvim
* IntelliJ IDEA + Rust plugin

=== Tu Primer Proyecto

Construirás tu primer proyecto y aprenderás:

* Estructura de un proyecto Rust (`src/`, `Cargo.toml`, `target/`)
* Compilación: `cargo build`, `cargo run`
* Perfiles: `--debug` vs `--release`
* Verificación rápida: `cargo check`

'''

== Módulo 1: Sintaxis Básica y Tipos

.*Objetivos*: Dominar la sintaxis fundamental de Rust

=== Variables y Conceptos Elementales

* Variables inmutables: `let`
* Variables mutables: `let mut`
* Constantes: `const`
* Variables estáticas: `static`
* Shadowing (sombreado de variables)
* Scope (alcance de las variables)

=== Tipos de Datos

==== Escalares
* Enteros: `i32`, `u32`, `i64`, etc.
* Flotantes: `f32`, `f64`
* Booleanos: `bool`
* Caracteres: `char`

==== Compuestos
* Tuplas: `(i32, f64, char)`
* Arrays: `[i32; 5]`
* Slices: `&[T]`

=== Manejo de Texto

* `String` vs `&str`
* Conversión entre tipos
* Métodos comunes de String

=== Conceptos de Memoria

* Stack vs Heap
* Cuándo se usa cada uno

=== Bloques y Expresiones

* Todo es expresión en Rust
* Bloques `{ }` retornan valores
* Diferencia entre expresión y sentencia

=== Documentación

* Comentarios: `//` y `/* */`
* Doc comments: `///` y `//!`
* Generar documentación: `cargo doc --open`

=== Macros de Uso Común

* `println!` y `print!`: Imprimir en consola
* `format!`: Formatear strings
* `dbg!`: Debugging rápido
* `todo!` y `unimplemented!`: Marcadores temporales
* `panic!`: Errores irrecuperables

'''

== Módulo 2: Ownership y Borrowing

.*Objetivos*: Entender el sistema de ownership (concepto pilar de Rust)

=== Conceptos de Memoria

* Stack vs Heap (profundización)
* Gestión de memoria sin garbage collector
* Move semantics
* RAII (Resource Acquisition Is Initialization)

=== Reglas de Ownership

* Cada valor tiene un único dueño
* Solo puede haber un dueño a la vez
* Cuando el dueño sale de scope, el valor se destruye

=== Borrowing (Préstamo)

* Referencias inmutables: `&T`
* Referencias mutables: `&mut T`
* Reglas del borrow checker:
** Múltiples `&T` O una `&mut T`
** No ambos al mismo tiempo

=== Lifetimes Básicos

* Lifetimes implícitos
* Cuándo Rust infiere lifetimes automáticamente

IMPORTANT: Ownership es el concepto MÁS IMPORTANTE de Rust. Dedica tiempo a entenderlo bien.

'''

== Módulo 3: Control de Flujo

.*Objetivos*: Dominar estructuras de control en Rust

=== Condicionales

* `if`, `else if`, `else`
* `if let` para pattern matching simplificado
* Expresiones condicionales (if como expresión)

=== Bucles

* `loop`: Bucle infinito
* `while`: Bucle condicional
* `for`: Iteración sobre colecciones
* `break` y `continue`
* Labels en bucles
* Retornar valores desde bucles

=== Pattern Matching

* `match`: Pattern matching exhaustivo
* Wildcards: `_`
* Match guards
* Destructuring en match

'''

== Módulo 4: Funciones

.*Objetivos*: Crear y usar funciones en Rust

=== Definición de Funciones

* Declaración y sintaxis
* Parámetros y tipos
* Valores de retorno
* Retorno implícito (última expresión)
* Funciones divergentes: `-> !`

=== Closures (Clausuras)

* Sintaxis: `|param| body`
* Captura de variables del entorno
* Traits: `Fn`, `FnMut`, `FnOnce`
* Move closures: `move ||`

=== Funciones Asociadas

* Métodos estáticos
* Constructores comunes

'''

== Módulo 5: Estructuras de Datos

.*Objetivos*: Modelar datos con structs y enums

=== Structs

* Classic structs: `struct Point { x: i32, y: i32 }`
* Tuple structs: `struct Color(u8, u8, u8);`
* Unit structs: `struct Marker;`
* Field init shorthand
* Struct update syntax

=== Métodos e Implementaciones

* Bloques `impl`
* Métodos: `&self`, `&mut self`, `self`
* Funciones asociadas (sin `self`)

=== Enumeraciones

* Definición de enums
* Variantes con datos
* Enums complejos
* Métodos en enums

=== Option<T>

* Representación de valores opcionales
* `Some(T)` y `None`
* Métodos útiles: `unwrap`, `unwrap_or`, `map`
* Pattern matching con `Option`

=== Vec<T> (Vector)

* Creación: `vec![]`, `Vec::new()`
* Métodos: `push`, `pop`, `len`
* Acceso: indexing vs `get()`
* Iteración sobre vectores

'''

== Módulo 6: Traits Básicos

.*Objetivos*: Entender y usar traits fundamentales

=== Traits Derivables

* `Debug`: Formato `{:?}` y `{:#?}`
* `Clone`: Copia explícita
* `Copy`: Copia implícita
* `PartialEq` y `Eq`: Comparación
* `Default`: Valor por defecto

=== Derivar Traits

* Sintaxis: `#[derive(Debug, Clone, PartialEq)]`
* Cuándo derivar vs implementar manualmente

=== Otros Traits Importantes

* `Display`: Formato `{}`
* `Drop`: Destructor automático

'''

== Módulo 7: Modularidad y Organización

.*Objetivos*: Estructurar proyectos grandes

=== Sistema de Módulos

* Declaración: `mod module_name`
* Jerarquía de módulos
* `src/lib.rs` vs `src/main.rs`
* Archivos y directorios como módulos

=== Visibilidad

* Privado por defecto
* `pub`: público
* `pub(crate)`: visible en el crate
* `pub(super)`: visible en el módulo padre

=== Uso e Importación

* `use` statement
* Re-exportación: `pub use`
* Glob imports: `use module::*;`
* Alias: `use std::io::Result as IoResult;`

=== Paths

* Paths absolutos: `crate::`
* Paths relativos: `self::`, `super::`

=== Workspaces y Crates

* Múltiples crates en un proyecto
* `Cargo.toml` del workspace
* Dependencias compartidas

'''

= NIVEL INTERMEDIO 🌿

== Módulo 8: Genéricos

.*Objetivos*: Escribir código reutilizable con genéricos

=== Funciones Genéricas

* Sintaxis: `fn foo<T>(x: T)`
* Múltiples parámetros de tipo
* Trait bounds: `where T: Display`

=== Structs Genéricas

* Definición: `struct Point<T> { x: T, y: T }`
* Implementaciones genéricas
* Restricciones de tipo

=== Enums Genéricos

* `Option<T>`, `Result<T, E>` (profundización)
* Crear tus propios enums genéricos

=== Lifetimes con Genéricos

* Combinar lifetimes y genéricos
* Sintaxis: `fn foo<'a, T>(x: &'a T)`

'''

== Módulo 9: Gestión de Errores

.*Objetivos*: Manejar errores de forma robusta

=== Errores Recuperables

* `Option<T>`: `Some(T)` y `None`
* `Result<T, E>`: `Ok(T)` y `Err(E)`
* Operador `?` para propagación

=== Errores Irrecuperables

* `panic!` y cuándo usarlo
* `unwrap` y `expect`
* Backtrace: `RUST_BACKTRACE=1`

=== Custom Error Types

* Crear tipos de error propios
* Implementar `std::error::Error`
* Enums como errores

=== Librerías de Errores

* *thiserror*: Derive macros para errors
* *anyhow*: Manejo simplificado de errores
* Cuándo usar cada una

'''

== Módulo 10: Colecciones Avanzadas

.*Objetivos*: Trabajar con estructuras de datos complejas

=== HashMap<K, V>

* Creación e inserción
* Acceso: `get()`, `[]`, `entry()`
* Iteración sobre keys/values/entries
* Actualización de valores

=== HashSet<T>

* Operaciones de conjuntos
* Unión, intersección, diferencia
* Membership testing

=== String y &str (Profundización)

* Encodings: UTF-8
* Indexación y por qué falla
* Slicing seguro
* Métodos de transformación

=== Otras Colecciones

* `VecDeque<T>`: Double-ended queue
* `LinkedList<T>`: Lista enlazada
* `BTreeMap` y `BTreeSet`: Ordenados
* `BinaryHeap<T>`: Priority queue

'''

== Módulo 11: Type Aliases y Newtypes

.*Objetivos*: Crear tipos personalizados

=== Type Aliases

* Sintaxis: `type Kilometers = i32;`
* Cuándo usarlos
* Type aliases para tipos complejos

=== Newtype Pattern

* Wrapper types: `struct UserId(u64);`
* Beneficios del newtype pattern
* Type safety adicional

=== Phantom Types

* `PhantomData<T>`
* Tipos fantasma para seguridad de tipos

'''

== Módulo 12: Iteradores

.*Objetivos*: Dominar el procesamiento de secuencias

=== Trait Iterator

* Implementar `Iterator`
* Método `next()`
* `IntoIterator`

=== Adaptadores

* `map`: Transformar elementos
* `filter`: Filtrar elementos
* `take`, `skip`: Limitar iteración
* `enumerate`: Obtener índices
* `zip`: Combinar iteradores

=== Consumidores

* `collect`: Construir colecciones
* `sum`, `product`: Agregar valores
* `fold`: Reducción personalizada
* `for_each`: Efectos secundarios

=== Iteradores Infinitos

* `std::iter::repeat`
* `std::iter::cycle`
* Cuándo usarlos

'''

== Módulo 13: Traits de Conversión

.*Objetivos*: Convertir entre tipos de forma idiomática

=== From y Into

* Trait `From<T>`
* Trait `Into<T>` (implementación automática)
* Conversiones sin fallo

=== TryFrom y TryInto

* Conversiones que pueden fallar
* Retornan `Result<T, E>`

=== AsRef y AsMut

* Referencias baratas
* `AsRef<T>` y `AsMut<T>`
* Cuándo usarlos

'''

= NIVEL AVANZADO 🌳

== Módulo 14: Traits Avanzados

.*Objetivos*: Dominar el sistema de traits de Rust

=== Associated Types

* `type Item;` en traits
* Diferencia vs genéricos
* Cuándo usar associated types

=== Trait Objects

* `dyn Trait`
* Fat pointers
* Object safety
* Limitaciones

=== Supertraits

* Requerir otros traits
* Sintaxis: `trait A: B + C`

=== Traits del Standard Library

* `Deref` y `DerefMut`
* Operator overloading: `Add`, `Sub`, `Mul`
* `Eq`, `PartialEq`, `Ord`, `PartialOrd`
* `Hash`

'''

== Módulo 15: Lifetimes Avanzados

.*Objetivos*: Dominar lifetimes complejos

=== Múltiples Lifetimes

* Relaciones entre lifetimes
* Coerción de lifetimes
* Lifetime bounds: `where T: 'a`

=== Higher-Rank Trait Bounds (HRTB)

* Sintaxis: `for<'a>`
* Casos de uso avanzados

=== Lifetime en Traits

* Associated types con lifetimes
* GATs (Generic Associated Types)

=== Subtyping y Variance

* Covariance
* Contravariance
* Invariance

'''

== Módulo 16: Smart Pointers

.*Objetivos*: Usar smart pointers avanzados

=== Box<T>

* Heap allocation
* Recursive types
* Trait objects

=== Rc<T> y Arc<T>

* `Rc<T>`: Reference counting (single-threaded)
* `Arc<T>`: Atomic reference counting (multi-threaded)
* `Weak<T>`: Referencias débiles

=== RefCell<T> y Cell<T>

* `RefCell<T>`: Interior mutability con verificación en runtime
* `Cell<T>`: Copy types con interior mutability
* Cuándo usar cada uno

'''

== Módulo 17: Concurrencia y Paralelismo

.*Objetivos*: Escribir código concurrente seguro

=== Threads

* `std::thread::spawn`
* `join()` para sincronización
* Ownership en threads
* Thread pools

=== Message Passing

* Channels: `mpsc`
* `Sender` y `Receiver`
* Sincronización con mensajes

=== Shared State

* `Mutex<T>`: Mutual exclusion
* `RwLock<T>`: Read-write lock
* `Arc<T>` para compartir entre threads
* Deadlocks y cómo evitarlos

=== Atomic Types

* `AtomicBool`, `AtomicI32`, `AtomicUsize`
* Ordering: `Relaxed`, `Acquire`, `Release`, `SeqCst`
* Lock-free programming

=== Traits Send y Sync

* `Send`: Tipos que se pueden mover entre threads
* `Sync`: Tipos que se pueden compartir entre threads

'''

== Módulo 18: Async/Await

.*Objetivos*: Escribir código asíncrono eficiente

=== Conceptos de Async

* Diferencia: concurrency vs parallelism
* Async vs threads
* Event loop y runtime

=== Futures

* Trait `Future`
* `Poll`, `Pending`, `Ready`
* Pin y Unpin

=== Sintaxis Async/Await

* `async fn`
* `async` blocks
* Palabra clave `.await`

=== Async Runtimes

* *tokio*: Runtime más usado
* *async-std*: Alternativa
* *smol*: Minimalista

=== Tasks y Spawning

* `tokio::spawn`
* Gestión de tareas
* Cancelación de tareas

=== Async I/O

* Reading y writing asíncrono
* Networking: TCP, UDP
* File I/O asíncrono

=== Streams

* `Stream` trait
* Procesamiento de flujos de datos
* Combinators

=== Select y Join

* `tokio::select!`: Esperar múltiples futures
* `tokio::join!`: Ejecutar futures concurrentemente

=== Channels Async

* `tokio::sync::mpsc`
* `tokio::sync::oneshot`
* `tokio::sync::broadcast`

=== Errores Comunes en Async

* Blocking en código async
* Too many tasks
* Memory leaks con referencias cíclicas

IMPORTANT: Async Rust es complejo. Practica mucho con proyectos reales.

'''

= NIVEL EXPERTO 🔥

== Módulo 19: Macros

.*Objetivos*: Crear macros declarativas y procedurales

=== Macros Declarativas

* `macro_rules!`
* Pattern matching en macros
* Repeticiones: `$(...)*`, `$(...)+`
* Hygiene

=== Macros Derivadas

* `#[derive(...)]` custom
* Crates `syn` y `quote`
* Parsing de AST

=== Attribute Macros

* `#[attribute]` custom
* Modificación de items

=== Function-like Macros

* `my_macro!(...)`
* Casos de uso

=== Debugging Macros

* `cargo expand`
* Expansión de macros
* Testing de macros

'''

== Módulo 20: Unsafe Rust y FFI

.*Objetivos*: Cuando necesitas romper las reglas

=== Unsafe Superpowers

* Dereferencing raw pointers
* Llamar funciones unsafe
* Acceder/modificar statics mutables
* Implementar unsafe traits
* Acceder a fields de unions

=== Raw Pointers

* `*const T` y `*mut T`
* Conversión desde referencias
* Cuándo usar raw pointers

=== Unsafe Traits

* Implementar `Send` y `Sync` manualmente
* Garantías de seguridad

=== Interior Mutability Unsafe

* `UnsafeCell<T>`
* Construcción de abstracciones seguras

=== FFI (Foreign Function Interface)

* Llamar código C desde Rust
* `extern "C"`
* `#[repr(C)]`
* Binding generation con `bindgen`

=== Llamar Rust desde Otros Lenguajes

* `#[no_mangle]`
* Crear bibliotecas dinámicas
* Exportar funciones

=== Librerías para FFI

* *cc*: Compilar C/C++ code
* *cmake*: Integración con CMake
* *pkg-config*

WARNING: Unsafe es peligroso. Solo úsalo cuando sea absolutamente necesario y documenta por qué es seguro.

'''

== Módulo 21: Testing, Benchmarking y Debugging

.*Objetivos*: Calidad y rendimiento del código

=== Unit Tests

* `#[cfg(test)]` y `#[test]`
* Assertions: `assert!`, `assert_eq!`, `assert_ne!`
* `cargo test`
* Organización de tests

=== Integration Tests

* Directorio `tests/`
* Testing de API pública
* Common modules

=== Doc Tests

* Tests en documentation comments
* `cargo test --doc`

=== Test Driven Development (TDD)

* Red-Green-Refactor cycle
* Test first approach

=== Property-Based Testing

* *proptest*: QuickCheck para Rust
* Generación automática de test cases

=== Mocking

* *mockall*: Framework de mocking
* Dependency injection

=== Benchmarking

* `cargo bench`
* *criterion*: Microbenchmarking
* Análisis estadístico
* Flamegraphs

=== Profiling

* `perf` en Linux
* Instruments en macOS
* `cargo flamegraph`
* Memory profiling

=== Debugging

* `dbg!` macro
* GDB y LLDB
* `rust-gdb` y `rust-lldb`
* VS Code debugger

'''

== Módulo 22: Optimización y Arquitectura

.*Objetivos*: Código production-ready de alto rendimiento

=== Performance Optimization

* Profiling primero
* Evitar allocations innecesarias
* Inline functions
* SIMD programming
* Cache-friendly code

=== Compilation Profiles

* Perfiles personalizados
* LTO (Link-Time Optimization)
* Codegen options
* Profile-guided optimization (PGO)

=== Binary Size Optimization

* Strip symbols
* `opt-level = "z"`
* `lto = true`
* Eliminar dependencias innecesarias

=== Arquitectura de Aplicaciones

* Layered architecture
* Hexagonal architecture
* Domain-driven design
* Clean architecture en Rust

=== Design Patterns

* Builder pattern
* Newtype pattern
* Type state pattern
* Strategy pattern
* Visitor pattern
